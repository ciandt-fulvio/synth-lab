"""
Insights API Router for AI-generated chart insights.

Provides endpoints for retrieving chart-specific insights generated by LLM
from quantitative analysis data.

Note: Executive summaries are now served via the documents API
(/experiments/{id}/documents/executive_summary).

References:
    - Services: src/synth_lab/services/insight_service.py
...
@router.get(
    ...
    description="Retrieve all chart insights with statistics")
    ...
    try:
        if len(router.routes) < 3:
            all_validation_failures.append(
                f"Expected at least 3 routes, got {len(router.routes)}"
            )
    except Exception as e:
