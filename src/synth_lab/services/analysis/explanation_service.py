...
Expected output:
    SegmentExplanation with segment_avg_adopted, population_avg_adopted,
...
class SegmentExplanation:
    ...
    segment_size: int
    segment_avg_adopted: float
    population_avg_adopted: float
...
def explain_segment(
    ...
    Args:
        synth_outcomes: List of synth outcome dicts with synth_id, adopted_rate,
    ...
    Returns:
        SegmentExplanation with adoption rates, differentiating factors, and explanation text.
    """
    ...
    if not segment_outcomes:
        ...
        return SegmentExplanation(
            segment_size=0,
            segment_avg_adopted=0.0,
            population_avg_adopted=_calculate_avg_adopted(population_outcomes),
            top_differentiating_factors=[],
            explanation_text="No synths found matching the specified segment IDs.",
        )

    # Calculate adoption rates
    segment_avg_adopted = _calculate_avg_adopted(segment_outcomes)
    population_avg_adopted = _calculate_avg_adopted(population_outcomes)
    ...
    explanation_text = _generate_explanation_text(
        ...
        segment_avg_adopted=segment_avg_adopted,
        population_avg_adopted=population_avg_adopted,
        factors=factors,
    )
    ...
    return SegmentExplanation(
        segment_size=len(segment_outcomes),
        segment_avg_adopted=round(segment_avg_adopted, 3),
        population_avg_adopted=round(population_avg_adopted, 3),
        top_differentiating_factors=factors,
        explanation_text=explanation_text,
    )


def _calculate_avg_adopted(outcomes: list[dict]) -> float:
    """Calculate average adoption rate across outcomes."""
    if not outcomes:
        return 0.0
    rates = [o.get("adopted_rate", 0.0) for o in outcomes]
...
def _generate_explanation_text(
    ...
    if not factors:
        return (
            f"Segment of {segment_size} synths has {segment_avg_adopted:.1%} adoption rate "
            f"(population: {population_avg_adopted:.1%}). No mechanism×sensitivity interactions found."
        )

    diff_pct = (segment_avg_adopted - population_avg_adopted) * 100
    direction = "higher" if diff_pct > 0 else "lower"

    lines = [
        f"Segment of {segment_size} synths has {segment_avg_adopted:.1%} adoption rate, "
        f"{abs(diff_pct):.1f}pp {direction} than population ({population_avg_adopted:.1%}).",
        "",
...
if __name__ == "__main__":
    ...
    try:
        ...
        if result.segment_avg_adopted != 0.75:
            all_validation_failures.append(
                f"segment_avg_adopted should be 0.75: {result.segment_avg_adopted}"
            )
        # Population includes all 3, so avg = (0.8 + 0.7 + 0.4) / 3 ≈ 0.633
        expected_pop = round((0.8 + 0.7 + 0.4) / 3, 3)
        if result.population_avg_adopted != expected_pop:
            all_validation_failures.append(
                f"population_avg_adopted should be {expected_pop}: {result.population_avg_adopted}"
            )
    ...
    try:
        basic_outcomes = [
            {"synth_id": "s1", "adopted_rate": 0.5, "synth_attributes": {}},
            {"synth_id": "s2", "adopted_rate": 0.6, "synth_attributes": {}},
        ]
        result = explain_segment(
    ...
    try:
        ...
        if "adoption rate" not in result.explanation_text.lower():
            all_validation_failures.append("Explanation should mention adoption rate")
