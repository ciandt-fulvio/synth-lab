...
from synth_lab.models.orm.simulation_run import SimulationReport as SimulationReportORM
...
class SimulationRunRepository(BaseRepository):
    ...
    def create_report(
        ...
        *,
        ...
        """Create a simulation report record.

        Args:
            report_id: Unique report ID (srp_xxx).
            experiment_id: Parent experiment ID.
        ...
        Returns:
            Created SimulationReportORM instance.
        """
        orm_report = SimulationReportORM(
            id=report_id,
            experiment_id=experiment_id,
            batch_id=batch_id,
            content=content,
        ...
        self._add(orm_report)
        self._flush()
        self._commit()
        return orm_report

    def get_report_by_batch(self, batch_id: str) -> SimulationReportORM | None:
        """Get the simulation report for a specific batch.

        Args:
            batch_id: Batch ID (sb_xxx).

        Returns:
            SimulationReportORM instance, or None if not yet generated.
        """
        stmt = (
            select(SimulationReportORM)
            .where(SimulationReportORM.batch_id == batch_id)
            .limit(1)
        )
