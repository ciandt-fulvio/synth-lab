...
export function RadarSection({ experimentId, hasClustering = true }: RadarSectionProps) {
  ...
  const radar = useAnalysisRadarComparison(experimentId);
  ...
  const handleGenerateInsight = () => {
    ...
    generateInsight.mutate({
      ...
      chartData: {
        ...
        attributes: radar.data.attributes,
      },
    });
  };
          ...
          >
            ...
            <div>
              <p className="text-body text-slate-500 font-medium mb-1">
                {hasClustering ? 'Sem Dados' : 'Clusters Necessários'}
              </p>
              <p className="text-meta">
                {hasClustering
                  ? 'Nenhum dado disponível para este gráfico.'
                  : 'Gere clusters primeiro para ver a comparação de perfis.'}
              </p>
            </div>
          </div>
        )}
